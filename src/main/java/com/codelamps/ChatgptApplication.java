...
public class ChatgptApplication {
    ...
    public static class MyGPTFunctions implements GPTFunctionsStub {
        ...
        public record SendEmailRequest(
            ...
            public String sendEmail(SendEmailRequest request) {
                System.out.println("Receipt : " + String.join(",", request.recipients()));
    ...
    ApplicationRunner runner(ChatGPTService service) {
        return args -> {
            var prompt = """
                    Would you please write an email to andrerizaldib@gmail.com and alice.aureliax@gmail.com
                    and invite them to attend the work in BNI on the next monday? Thanks in advance.
                    """
                    .trim();
            ...
            service.chat(ChatCompletionRequest.functions(prompt, List.of("send_email")))
                    .map(ChatCompletionResponse::getReplyCombinedText)
                    .subscribe(System.out::println);
        };
    }
